...
import {
  useEntityRiskAssessment,
  useEntityRiskHistory,
...
interface EntityRiskAssessmentTabProps {
  entityId: string;
}

export function EntityRiskAssessmentTab({ entityId }: EntityRiskAssessmentTabProps) {
  const { t } = useTranslation();
  const riskQuery = useEntityRiskAssessment(entityId);
  const historyQuery = useEntityRiskHistory(entityId);
  ...
  const handleCalculate = () => {
    calculateMutation.mutate(entityId);
  };
